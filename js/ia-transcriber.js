...
      els.ticketList.innerHTML = `
        <div class="ia-image-empty flex flex-col items-center justify-center p-8 text-center text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-gray-200 dark:border-slate-700">
          <i data-lucide="inbox" class="w-8 h-8 mb-3 opacity-50"></i>
          <span class="text-sm font-medium">Nenhum ticket encontrado</span>
          <span class="text-xs opacity-75 mt-1">Busque outro termo ou crie um novo.</span>
        </div>`;
      return;
    }
...
        const title = escapeHtml(ticket.customName || ticket.phone || "Sem nome");
        // UX: Destaque visual suave para o ticket ativo e hover elegante para os inativos
        const activeClass = ticket.id === state.activeId 
          ? "active bg-sky-50/60 border-sky-300 shadow-sm ring-1 ring-sky-200 dark:bg-sky-900/20 dark:border-sky-800" 
          : "bg-white border-transparent hover:border-gray-200 hover:bg-gray-50 dark:bg-transparent dark:hover:bg-slate-800/60";
          
        const statusIcon = ticket.isRegistered
          ? '<i data-lucide="check-circle-2" class="w-4 h-4 text-green-500 drop-shadow-sm flex-shrink-0"></i>'
          : '<i data-lucide="ticket" class="w-4 h-4 text-sky-500 drop-shadow-sm flex-shrink-0"></i>';

        return `
        <article class="ia-ticket-item group flex flex-col p-3 mb-2 rounded-xl border transition-all duration-200 cursor-pointer ${activeClass}" data-ticket-id="${ticket.id}">
          <div class="ia-ticket-line flex items-start justify-between gap-3 w-full">
            <div class="ia-ticket-main flex-1 min-w-0">
              <div class="ia-ticket-title flex items-center gap-2 text-sm font-semibold text-gray-800 dark:text-gray-100 mb-0.5 truncate">
                ${statusIcon}
                <span class="truncate">${title}</span>
              </div>
              <div class="ia-ticket-meta flex items-center gap-1 text-xs text-gray-400 dark:text-gray-500 font-medium">
                <i data-lucide="clock" class="w-3 h-3"></i>
                ${escapeHtml(ticket.createdAt || "")}
              </div>
            </div>

            <div class="ia-ticket-actions flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              <button class="ia-icon-btn p-1.5 text-gray-400 hover:text-sky-600 hover:bg-sky-100 dark:hover:bg-sky-900/40 rounded-lg transition-colors" type="button" data-action="rename" data-ticket-id="${ticket.id}" title="Renomear">
                <i data-lucide="square-pen" class="w-4 h-4"></i>
              </button>
              <button class="ia-icon-btn ia-icon-danger p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" type="button" data-action="delete" data-ticket-id="${ticket.id}" title="Excluir">
...
              ? `
            <div class="mt-3 animate-in fade-in duration-200">
              <input class="ia-ticket-input w-full text-sm px-3 py-2 bg-white dark:bg-slate-900 border border-sky-300 dark:border-sky-600 rounded-lg shadow-inner focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition-all" data-ticket-edit-input="${ticket.id}" placeholder="Digite e tecle Enter..." value="${escapeAttribute(
...
    const contact = ticket.customName ? `${ticket.customName} (${phone})` : `(${phone})`;
    ...
    // UX: Relatório estlizado estruturalmente inline para garantir uma boa
    // formatação caso seja colado dentro de emails ou no rich-text do Znuny.
    return [
      '<div style="font-family: sans-serif; font-size: 14px; line-height: 1.5; color: #333;">',
        '<span style="color:#f39c12; font-size: 12px; text-transform: uppercase;"><strong>PROBLEMA / DÚVIDA:</strong></span><br />',
        `<span style="display: inline-block; margin-top: 4px;">Em contato com usuário <b>${escapeHtml(contact)}</b>.</span><br />`,
        `<span style="display: block; margin-top: 6px; padding: 10px; background-color: #f8f9fa; border-left: 3px solid #f39c12; border-radius: 4px;">${escapeHtml(ticket.analysis || "Aguardando transcrição...")}</span><br />`,
        '<span style="color:#4dabf7; font-size: 12px; text-transform: uppercase;"><strong>ENCAMINHAMENTO / SOLUÇÃO:</strong></span><br />',
        `<span style="display: block; margin-top: 6px; padding: 10px; background-color: #f8f9fa; border-left: 3px solid #4dabf7; border-radius: 4px;">${escapeHtml(ticket.solucao || "Aguardando transcrição...")}</span>`,
      '</div>'
    ].join("");
  }
